...
def create_assistant(file):
    ...
    assistant = client.beta.assistants.create(
        name="WhatsApp Airbnb Assistant 2",
...
def generate_response(message_body, wa_id, name):
    ...
    if thread_id is None:
        print(f"Creating a new thread for {name} with wa_id {wa_id}")
        thread = client.beta.threads.create()
        store_thread(wa_id, thread.id)
        thread_id = thread.id

    #  Otherwise, add the message to the existing thread
    else:
    ...
    # Run the assistant and get new message
